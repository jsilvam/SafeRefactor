...
//		this.dw = new DesignWizard(DESIGNWIZARD_DIR);
	...
	public void testgetInheritedMethodsNotIncludePrivates() throws IOException, InexistentEntityException {
		this.dw = new DesignWizard("test_resources"+File.separator+"jars"+File.separator+"TestInheritance.jar");
		
		
		ClassNode subClass = this.dw.getClass("SubAndSuper");
		assertEquals(subClass, this.dw.getClass("SubAndSuper"));
		Set<MethodNode> inheritedMethods = subClass.getInheritedMethods();
		for (MethodNode method : inheritedMethods) {
				assertFalse(method.getName().equals("Super.k(int)"));
		}				
	}
	
	public void testgetInheritedMethodsNotIncludeOverridenMethods() throws IOException, InexistentEntityException {
		this.dw = new DesignWizard("test_resources"+File.separator+"jars"+File.separator+"TestInheritance.jar");
		
		
		ClassNode subClass = this.dw.getClass("Sub");
		assertEquals(subClass, this.dw.getClass("Sub"));
		Set<MethodNode> inheritedMethods = subClass.getInheritedMethods();
		System.out.println(inheritedMethods);
		for (MethodNode method : inheritedMethods) {
				assertFalse(method.getName().equals("Super.k()"));
		}				
		
	}
	
	public void testgetOverloadedMethods() throws IOException, InexistentEntityException {
		this.dw = new DesignWizard("test_resources"+File.separator+"jars"+File.separator+"TestOverloading.jar");
		
		MethodNode overloadedMethod1 = this.dw.getMethod("Super.k()");
		MethodNode overloadedMethod2 = this.dw.getMethod("Super.k(int)");
		
		ClassNode clazz = this.dw.getClass("Super");
		assertEquals(clazz , this.dw.getClass("Super"));
		
		Set<MethodNode> overloadedMethods = clazz.getOverloadedMethods();
		assertTrue(overloadedMethods.contains(overloadedMethod1));
		assertTrue(overloadedMethods.contains(overloadedMethod2));
		assertTrue(overloadedMethods.size() == 2);
	}
	
	public void testgetOverloadedInheritedMethods() throws IOException, InexistentEntityException {
		this.dw = new DesignWizard("test_resources"+File.separator+"jars"+File.separator+"TestOverloading.jar");
		
		MethodNode overloadedMethod1 = this.dw.getMethod("Super.k()");
		MethodNode overloadedMethod2 = this.dw.getMethod("Super.k(int)");
		MethodNode overloadedMethod3 = this.dw.getMethod("SubAndSuper.k(long)");
		
		ClassNode clazz = this.dw.getClass("SubAndSuper");
		assertEquals(clazz , this.dw.getClass("SubAndSuper"));
		
		Set<MethodNode> overloadedMethods = clazz.getOverloadedMethods();
		assertTrue(overloadedMethods.contains(overloadedMethod1));
		assertTrue(overloadedMethods.contains(overloadedMethod2));
		assertTrue(overloadedMethods.contains(overloadedMethod3));
		assertTrue(overloadedMethods.size() == 3);
	}
	
	
	public void testgetOverloadedInheritedMethodsThreeLevelHierarchy() throws IOException, InexistentEntityException {
		this.dw = new DesignWizard("test_resources"+File.separator+"jars"+File.separator+"TestOverloading.jar");
		
		MethodNode overloadedMethod1 = this.dw.getMethod("Super.k()");
		MethodNode overloadedMethod2 = this.dw.getMethod("Super.k(int)");
		MethodNode overloadedMethod3 = this.dw.getMethod("SubAndSuper.k(long)");
		MethodNode overloadedMethod4 = this.dw.getMethod("Sub.k(int,int)");
		
		ClassNode clazz = this.dw.getClass("Sub");
		assertEquals(clazz , this.dw.getClass("Sub"));
		
		Set<MethodNode> overloadedMethods = clazz.getOverloadedMethods();
		assertTrue(overloadedMethods.contains(overloadedMethod1));
		assertTrue(overloadedMethods.contains(overloadedMethod2));
		assertTrue(overloadedMethods.contains(overloadedMethod3));
		assertTrue(overloadedMethods.contains(overloadedMethod4));
		assertTrue(overloadedMethods.size() == 4);
	}
