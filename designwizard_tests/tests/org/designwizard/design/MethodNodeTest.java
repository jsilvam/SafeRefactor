...
public class MethodNodeTest extends TestCase {
	...
	public void testGetPackageName() throws IOException,
		...
		for (MethodNode method : classNode.getAllMethods()) {
			assertEquals("org.designwizard.common", method.getPackage()
					.getName());
		}

		classNode = this.dw.getClass(AbstractElementsFactory.class);
		for (MethodNode method : classNode.getAllMethods()) {
			assertEquals("org.designwizard.design.factory", method.getPackage()
					.getName());
		}

		classNode = this.dw.getClass(ClassNodeTest.class);
		for (MethodNode method : classNode.getAllMethods()) {
			assertEquals("tests.org.designwizard.design", method.getPackage()
					.getName());
		}
		...
		this.dw = new DesignWizard("resources" + File.separator + "testFiles"
				+ File.separator + "visibility.jar");
	...
	public void testGetOverridenMethod() throws IOException,
			InexistentEntityException {

		this.dw = new DesignWizard("test_resources" + File.separator + "jars"
				+ File.separator + "TestInheritance.jar");

		ClassNode classNode = this.dw.getClass("SubAndSuper");

		MethodNode overridenMethod = null;
		for (MethodNode method : classNode.getAllMethods()) {
			if (method.getName().equals("SubAndSuper.k()")) {
				overridenMethod = method.getOverridenMethod();

			}
		}
		assertEquals("Super.k()", overridenMethod.getName());

	}

	public void testGetOverridenMethodWithThreeLevelHierarchy()
			throws IOException, InexistentEntityException {

		this.dw = new DesignWizard("test_resources" + File.separator + "jars"
				+ File.separator + "TestOverriding.jar");

		ClassNode classNode = this.dw.getClass("Sub");

		MethodNode overridenMethod = null;
		MethodNode secondOverridenMethod = null;
		for (MethodNode method : classNode.getAllMethods()) {
			if (method.getName().equals("Sub.k(int)")) {
				overridenMethod = method.getOverridenMethod();
			}
			if (method.getName().equals("Sub.k()")) {
				secondOverridenMethod = method.getOverridenMethod();
			}
		}
		assertEquals("Super.k(int)", overridenMethod.getName());
		assertEquals("SubAndSuper.k()", secondOverridenMethod.getName());

	}

	public void testGetOverridenBy() throws IOException,
			InexistentEntityException {

		this.dw = new DesignWizard("test_resources" + File.separator + "jars"
				+ File.separator + "TestInheritance.jar");

		ClassNode classNode = this.dw.getClass("Super");
		MethodNode methodThatOverridesIt = null;
		for (MethodNode method : classNode.getAllMethods()) {
			if (method.getName().equals("Super.k()")) {
				methodThatOverridesIt = method.getOverridenBy();
			}
		}
		assertEquals("SubAndSuper.k()", methodThatOverridesIt.getName());
	}

	public void testGetOverridenByThreeLevelHierarchy() throws IOException,
			InexistentEntityException {
		...
		ClassNode classNode = this.dw.getClass("Super");
		MethodNode methodThatOverridesIt = null;
		MethodNode methodThatOverridesIt2 = null;
		for (MethodNode method : classNode.getAllMethods()) {
			if (method.getName().equals("Super.k(int)")) {
				methodThatOverridesIt = method.getOverridenBy();
			}
			if (method.getName().equals("Super.k()")) {
				methodThatOverridesIt2 = method.getOverridenBy();
			}
		}
		assertEquals("Sub.k(int)", methodThatOverridesIt.getName());
		assertEquals("SubAndSuper.k()", methodThatOverridesIt2.getName());
	}
	...
	public void testGetShortName() throws IOException,
		...
		MethodNode method = dw
				.getMethod("org.designwizard.design.AbstractEntity.getShortName()");
		assertEquals("getShortName", method.getShortName());
		assertEquals(method.getShortName(), method.getShortName());

		method = dw
				.getMethod("org.designwizard.design.Entity.addModifier(org.designwizard.design.Modifier)");
		assertEquals("addModifier", method.getShortName());

	}

	public void testReturnMethod() throws InexistentEntityException,
			IOException {

		this.dw = new DesignWizard("resources" + File.separator + "testFiles"
				+ File.separator + "returnmethod.jar");
	...
	public void testMainThrows() throws InexistentEntityException, IOException {
		...
		this.dw = new DesignWizard("resources" + File.separator + "testFiles"
				+ File.separator + "throws.jar");
		MethodNode method = this.dw
				.getMethod("Throws.main(java.lang.String[])");
	...
	public void testGetAllMethodsWithVisibility() throws IOException,
			InvalidTypeOfVisibility, InexistentEntityException {

		this.dw = new DesignWizard("resources" + File.separator + "testFiles"
				+ File.separator + "accessmodifiers.jar");
