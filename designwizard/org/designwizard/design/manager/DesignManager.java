...
public class DesignManager implements ExtractionListener {
	...
	/**
	 * Creates a new DesignManager. An instance of this class is responsible by
	 * receiving the extraction events and fill the <code>Design</code> object.
	 * 
	 * @param translator
	 */
	...
	/**
	 * Extracts fact from the given bytecode. The path must be a jar file, or
	 * the classes directory from project.
	 * 
	 * @param path
	 ...
	 */
	public void extractFacts(String path) throws IOException,
	...
	public void extractFacts(String path, String... internalPath)
		...
		if (file.getName().endsWith(".war")
			...
			throw new IOException(
					"Path does not correspond to a directory or file a valid.");
		}
	}
	 ...
	 *             If an I/O error occurs.
	 */
	private void extractFactsFromEar(String path, String... internalPath)
			throws IOException {
		for (Extractor extractor : this.extractors) {
	 ...
	 *             If an I/O error occurs.
	 */
	private void extractFactsFromClassesDir(String pathOfClassesDir)
			throws IOException {
		for (Extractor extractor : this.extractors) {
	 ...
	 * 
	 * @param type
	 *            the type of the <code>Relation</code>.
	 * @param caller
	 *            the caller of the <code>Relation</code>.
	 * @param called
	 *            the called of the <code>Relation</code>.
	 */
	private void addRelation(TypesOfRelation type, String caller, String called) {
		String toPutCaller = caller;

		String toPutCalled = "";
		if (type == TypesOfRelation.EXTENDS) {
			...
			if (caller.length() != 1) {
				toPutCaller = this.translator
						.translateBytecodeToJavaPattern(caller);
			}
			toPutCalled = this.translator
					.translateBytecodeToJavaPattern(called);
		}
	...
	public void relationExtracted(FactEvent event) {
		try {
			this.addRelation(TypesOfRelation.extractElement(event.getType()),
					event.getCaller(), event.getCalled());
		} catch (InexistentTypeOfRelationException e) {
			// TODO if the follow line is processed, the extractor is not
			// functioning properly. The type of relation is invalid.
			e.printStackTrace();
		}
	}
	...
	public void visibilityExtracted(FactEvent event) {
		...
		try {
			...
			this.design.setVisibility(
					event.getEntity().replace(DesignIF.FIELD_IDENTIFIER, ""),
					type);
	...
	/**
	 * Event generated by the extractor. When a signature of method is
	 * extracted.
	 */
	public void signatureExtracted(FactEvent event) {
		...
		try {
			...
			this.setSignatureOfMethod(entity,
					Modifier.extractModifier(event.getModifier()));
			...
			// TODO if the follow line is processed, the extractor is not
			// functioning properly. The type of modifier is invalid.
			e.printStackTrace();
	...
	private void setSignatureOfMethod(String method, Modifier visibility) {
		...
		String methodName = this.translator
				.translateBytecodeToJavaPattern(method);
		String parameters = this.translator.extractSignature(sig.substring(1,
				sig.indexOf(")")));
		String returnType = this.translator.translateBytecodeToJavaPattern(sig
				.substring(sig.indexOf(")") + 1));

		try {

			this.design.setReturnType(
					methodName.replace(DesignIF.METHOD_IDENTIFIER, ""),
					returnType);
			this.design.setVisibility(
					methodName.replace(DesignIF.METHOD_IDENTIFIER, ""),
					visibility);
			this.design.setParameters(
					methodName.replace(DesignIF.METHOD_IDENTIFIER, ""),
					parameters);
	...
	/**
	 * Analyze the impact of a change in the <code>Entity</code> object
	 * specified by the entityName.
	 * 
	 * This method fill the <code>ResultManager</code> object with the impact of
	 * a change in the <code>Entity</code> object specified by the entityName.
	 * To see the result of the analyzed entities, get the
	 * <code>ResultManager</code> object invoking the
	 * getResultOfAnalyzedEntities() of this class.
	 * 
	 */
	public void analyzeImpact(String entityName) {
		...
		try {
			...
			this.resultOfImpact.addTrace(entityName,
					entity.getImpactOfAChange());
	 ...
	 * 
	 * @return the result generated by the analyzeImpact(String entityName)
	 *         method.
	 */
	...
	public void modifiersExtracted(FactEvent event) {
		...
		try {
			...
			if (event.getEntity().replace(DesignIF.METHOD_IDENTIFIER, "")
					.length() != 1) {

				String entityID = this.translator
						.translateBytecodeToJavaPattern(event.getEntity())
						.replace(DesignIF.METHOD_IDENTIFIER, "");

				entity = this.design.getEntity(entityID);

			} else {

				entity = this.design.getEntity(event.getEntity().replace(
