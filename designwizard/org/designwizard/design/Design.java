 ...
 * 
 *         This class represents a model for the facts extracts from .jar file
 *         and source code.
 */
public class Design implements DesignIF {
	...
	/**
	 * Constructs a new Design. A design is a set of entities and all the
	 * relations between this entities.
	 */
	public Design() {
	 ...
	 * 
	 * @param className
	 *            the name of the class extracted.
	 */
	 ...
	 * 
	 * @param relation
	 *            the relation to be added. pattern: - Methods:
	 *            M:Class.method(int,double,...) - Fields: F:Class.fieldName -
	 *            Class: package.subpackage.ClassName
	 * 
	 *            ps1.: Varargs must be describe as an array. ps2.: It is not
	 *            necessary to describe generics. ps3.: put S: for static
	 *            methods. Example: S:M:Main.main(String[])
	 * */
	public void addRelation(TypesOfRelation typeOfRelation, String callerName,
			String calledName) {

		Entity caller = null, called = null;
		caller = factory.createEntity(callerName);
		called = factory.createEntity(calledName);
		Relation relation = factory.createRelation(typeOfRelation, caller,
				called);

		this.addNewEntities(caller, called, relation);

		Relation reverseRelation = this.resolveReverseRelation(caller, called,
				typeOfRelation);
		if (reverseRelation != null)
			this.addNewEntities(called, caller, reverseRelation);

	}

	/**
	 * Creates the reverse relation for the entities and adds them in the
	 * Design.
	 * 
	 * @return The reverse relation or null if the there is no reverse relation
	 *         for the specified type.
	 */
	// TODO especificar quais s�o as reverse relations.
	private Relation resolveReverseRelation(Entity caller, Entity called,
			TypesOfRelation typeOfRelation) {
		TypesOfRelation typeReverseRelation = typeOfRelation
				.getReverseRelation();
		return (typeReverseRelation != null) ? factory.createRelation(
				typeReverseRelation, called, caller) : null;
	}
	 ...
	 * 
	 * @param caller
	 *            the entity that will be added as a caller of the specified
	 *            relation.
	 * @param called
	 *            the entity that will be added as a called of the specified
	 *            relation.
	 * @param relation
	 *            the relation to be added.
	 * @param typeOfRelationString
	 *            the type of the relation to be added.
	 */
	...
	public void resolveDependences() {
		try {
			resolveInheritedFieldsAndMethods(this.getClass(Object.class
					.getName()));
			resolveOverloadingMethods(this.getClass(Object.class.getName()));
	...
	private void resolveOverloadingMethods(ClassNode classNode) {

		Set<ClassNode> subClasses = classNode.getSubClasses();
		
		for (ClassNode subClass : subClasses) {
			Set<MethodNode> allMethods = subClass.getAllMethods();

			Set<MethodNode> overloadedMethods = new HashSet<MethodNode>();
			for (MethodNode method : allMethods) {
				String name = method.getShortName();
				Set<MethodNode> methodsByName = getMethodsByName(name,
						allMethods);
				if (methodsByName.size() > 1)
					overloadedMethods.addAll(methodsByName);
			}
			subClass.setOverloadedMethods(overloadedMethods);
			resolveOverloadingMethods(subClass);
		}

	}

	private Set<MethodNode> getMethodsByName(String name,
			Set<MethodNode> allMethods) {
		Set<MethodNode> result = new HashSet<MethodNode>();

		for (MethodNode methodNode : allMethods) {
			if (methodNode.getShortName().equals(name))
	...
	private void resolveInterfaces() {
		for (ClassNode classNode : this.getAllClasses()) {
			if (classNode.isInterface()) {
				try {
					Set<ClassNode> implementsIf = classNode
							.getEntitiesThatImplements();
					for (ClassNode impl : implementsIf) {
						if (impl.isInterface()) {
							for (MethodNode methodNode : classNode
									.getAllMethods()) {
								MethodNode aux = (MethodNode) this.factory
										.createEntity(DesignIF.METHOD_IDENTIFIER
												+ impl.getName()
												+ "."
												+ methodNode.getShortName());
								this.addNewEntities(impl, aux, factory
										.createRelation(
												TypesOfRelation.CONTAINS, impl,
												aux));
								this.addNewEntities(aux, impl, this
										.resolveReverseRelation(impl, aux,
												TypesOfRelation.CONTAINS));
								...
								impl.addInheritedMethod(this.getMethod(impl
										.getName()
										+ "."
										+ methodNode.getShortName()));
							}
	...
	/**
	 * Adds the inherited fields and methods on the subclasses of
	 * <code>ClassNode</code>.
	 */
	private void resolveInheritedFieldsAndMethods(ClassNode classNode) {
		...
		for (ClassNode subClass : subClasses) {
			...
			for (MethodNode methodNode : methods) {
				...
				if (isMethodImplementedByClass(methodNode, subClass))
					continue;
	...
	private boolean isMethodImplementedByClass(MethodNode methodNode,
			ClassNode subClass) {

		Set<MethodNode> methods = subClass.getAllMethods();

		for (MethodNode comparedMethod : methods) {
			List<ClassNode> comparedParameters = comparedMethod.getParameters();
			List<ClassNode> parameters = methodNode.getParameters();

			if (methodNode.getShortName().equals(comparedMethod.getShortName())
					&& comparedParameters.equals(parameters)) {
				comparedMethod.setOverridenMethod(methodNode);
				methodNode.setOverridenBy(comparedMethod);
				return true;
			}
	...
	private boolean isPrivateMethod(MethodNode method) {
		...
		for (Entity entity : this.entities.values()) {
			if (entity.getTypeOfEntity().equals(Entity.TypesOfEntities.METHOD)) {
				...
				for (Relation relation : relations) {
					if (!relation.getCalledEntity().getTypeOfEntity()
							.equals(TypesOfEntities.CLASS)) {
						...
						if (withoutParentClass.getClassNode() == null) {
							...
							if (this.isClassFromCode(className)) {
								try {
									...
									if (withoutParentClass.getName().contains(
											"$")
											|| superClass == null
											|| !this.isClassFromCode(superClass
													.getName()))
										continue;

									entity.removeRelation(relation);
									Relation reverseRelation = this
											.resolveReverseRelation(
													relation.getCallerEntity(),
													relation.getCalledEntity(),
													relation.getType());
									entity.removeRelation(reverseRelation);

									if (withoutParentClass.getTypeOfEntity()
											.equals(TypesOfEntities.METHOD)) {
										fromSuperClass = superClass
												.getDeclaredMethod(withoutParentClass
														.getShortName());

										if (fromSuperClass != null)
											this.addRelation(
													relation.getType(),
													DesignIF.METHOD_IDENTIFIER
															+ method.getName(),
													DesignIF.METHOD_IDENTIFIER
															+ fromSuperClass
																	.getName());
										...
										fromSuperClass = superClass
												.getField(withoutParentClass
														.getShortName());
										this.addRelation(
												relation.getType(),
												DesignIF.METHOD_IDENTIFIER
														+ method.getName(),
												DesignIF.FIELD_IDENTIFIER
														+ fromSuperClass
																.getName());
									}
								} catch (InexistentEntityException e) {
	...
	/**
	 * Verifies whether the class with the specified className if from the code
	 * extracted.
	 */
	 ...
	 * @return true if the entity with the name equals the parameter exists in
	 *         this <code>Design</code> or false if not.
	 */
	 ...
	 * 
	 * @return a set containing all classes extracted. * If no class was
	 *         extracted, this method will return a empty Set<String>;
	 */

	/*
	 * (non-Javadoc)
	 * 
	 * @see designWizard.design.DesignIF#setVisibility(java.lang.String,
	 * designWizard.design.TypesOfVisibility)
	 */
	public void setVisibility(String entityName, Modifier visibility)
	 ...
	 * (non-Javadoc)
	 * 
	 * @see designWizard.design.DesignIF#setReturnType(java.lang.String,
	 * java.lang.String)
	 */
	public void setReturnType(String method, String returnType)
			throws InexistentEntityException {
	...
	public void setParameters(String method, String parameters)
			throws InexistentEntityException {

		
		MethodNode methodNode = this.getMethod(method);
		ClassNode parameter;
		List<ClassNode> methodParameters = new ArrayList<ClassNode>();
	...
	public Entity getEntity(String entityName) throws InexistentEntityException {
		if (this.entities.get(entityName) == null)
			throw new InexistentEntityException(entityName);
		else
	...
	/**
	 * Returns a <code>Set</code> of <code>ClassNode</code> objects representing
	 * all classes extracted. This includes those classes from java and
	 * libraries used by the code extracted. Realize that classes from java are
	 * not entirely extracted. If you need only the classes from your source
	 * code, you must call the
	 * {@link designwizard.design.Design#getAllClassesFromCode()}.
	 * 
	 * @return a <code>Set</code> of <code>ClassNode</code> objects representing
	 *         all the classes extracted.
	 */
	...
	public Set<PackageNode> getAllPackagesFromCode() {
		...
		for (Entity entity : this.entities.values()) {
			if (entity.getTypeOfEntity().equals(TypesOfEntities.PACKAGE))
				feedBack.add((PackageNode) entity);
		}
		return feedBack;
	}

	@Override
	public PackageNode getPackage(String fullyQualifiedNamePackage)
			throws InexistentEntityException {
